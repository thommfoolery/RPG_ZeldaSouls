...
func _ready() -> void:
	...
	# ─── CRITICAL: Restore permanent state first ───
	if WorldStateManager and WorldStateManager.is_permanent("opened_chests", chest_id):
		_open_chest(true)          # silent = true → no extra prints on load
		print("[Chest] Loaded as OPEN (persistent) → ", chest_id)
	else:
		_set_closed_state()        # explicit closed state
...
func _set_closed_state() -> void:
	closed_sprite.visible = true
	open_sprite.visible = false
	collision_layer = 1          # your wall/door layer
	is_opened = false
...
func _enable_linked_loot() -> void:
	if linked_item_pickup_id.is_empty():
		print("[Chest] No linked_item_pickup_id set on ", chest_id)
		return
