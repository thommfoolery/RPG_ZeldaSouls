...
func _ready() -> void:
	print("[ATTUNE-MENU] _ready() — Pre-made 9 slots with unique backgrounds loaded")
	...
	# Set empty background on all slots
...
func _input(event: InputEvent) -> void:
	...
	print("[ATTUNE-MENU] [INPUT] Event: ", event.as_text(), " | selection_mode=", is_in_selection_mode,
		  " | attune_slot=", selected_attunement_index, " | spell_idx=", selected_spell_index)

	# B / ui_cancel always closes the whole menu (critical!)
	...
	if is_in_selection_mode:
		...
		if event.is_action_pressed("ui_right") or event.is_action_pressed("cycle_right_hand"):
			...
			print("[ATTUNE-MENU] Confirm (A) pressed in selection mode")
	...
	# Normal mode - top row Left/Right only
	if event.is_action_pressed("ui_right") or event.is_action_pressed("cycle_right_hand"):
		selected_attunement_index = (selected_attunement_index + 1) % PlayerStats.attunement_slots_unlocked
		print("[ATTUNE-MENU] Moved RIGHT → slot ", selected_attunement_index)
		_highlight_current_attunement_slot()
		get_viewport().set_input_as_handled()
	elif event.is_action_pressed("ui_left") or event.is_action_pressed("cycle_left_hand"):
		...
		print("[ATTUNE-MENU] Moved LEFT → slot ", selected_attunement_index)
		_highlight_current_attunement_slot()
		get_viewport().set_input_as_handled()
	elif event.is_action_pressed("ui_accept") or event.is_action_pressed("interact"):
		if PlayerStats.attunement_slots_unlocked > 0:
			...
			print("[ATTUNE-MENU] A pressed → ENTERED selection mode on slot ", selected_attunement_index)
			_highlight_current_spell()
			get_viewport().set_input_as_handled()
...
func open_attune_menu() -> void:
	...
	_clear_bottom_description()
	_highlight_current_attunement_slot()
	print("[ATTUNE-MENU] open_attune_menu() called — menu fully opened and highlighted")
