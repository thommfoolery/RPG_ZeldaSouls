...
# ─── UNIQUE QTY LABELS (exactly matching the names you added) ───
@onready var qty_labels: Array[Label] = [
	null, null,                                   # RightHand1 / RightHand2
	...
	null, null,                                   # LeftHand1 / LeftHand2
	$MainContainer/GridContainer/Ammo1/Ammo1Qty,
	$MainContainer/GridContainer/Ammo2/Ammo2Qty,
	null, null, null, null,                       # Armor slots
	null, null, null                              # Ring slots
]

# YOUR EXACT GOLDEN NEIGHBOR MAP - FULLY RESTORED
const NEIGHBORS = {
...
func _ready() -> void:
	print("[EQUIP-TAB] Golden navigation + Assign Menu + X Unequip + Armor Slot Filtering + UNIQUE QTY labels")
...
func _input(event: InputEvent) -> void:
	...
	if is_assign_menu_open:
		...
		if event.is_action_pressed("ui_cancel"):
			...
			get_viewport().set_input_as_handled()
			return
		# DO NOT consume ui_accept here — let ItemList fire its own activation signal
...
# (the rest of your script — _open_assign_menu, _close_assign_menu, _on_item_list_*, _unequip_current_slot, helpers, _update_all_slots, equip_item — stays exactly the same)
