...
func _ready() -> void:
	# ── CRITICAL: FORCE HIDDEN ON EVERY LOAD ──
	visible = false
	...
	print("[MENU-DEBUG] CharacterMenu FORCED hidden on load | Global.is_in_menu = false | input_blocked = false")
...
func _on_menu_toggled() -> void:
	# Extra safety: never open if bonfire menu (or any other UI) is active
	if UIManager and UIManager.active_menus.has("bonfire"):
		print("[MENU-DEBUG] _on_menu_toggled BLOCKED - Bonfire menu is open")
		return
	...
	print("[MENU-DEBUG] CharacterMenu toggled: ", "OPEN" if visible else "CLOSED", " | blocked=", InputManager.input_blocked if InputManager else "null")
...
func _on_visibility_changed() -> void:
	...
	if visible:
		...
		menu_closed.emit()
		if UIManager:
			UIManager.character_menu_closed.emit()
...
# ────────────────────────────────────────────────────────────────
# INPUT - Extra ghost protection
# ────────────────────────────────────────────────────────────────
func _input(event: InputEvent) -> void:
	if not visible:
		# Ghost A protection after menu close
		if _just_closed_this_frame and (event.is_action_pressed("ui_accept") or event.is_action_pressed("interact")):
			get_viewport().set_input_as_handled()
			print("[MENU-DEBUG] Ate ghost ui_accept / interact after menu close")
		return
