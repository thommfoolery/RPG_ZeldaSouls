...
var ignore_b_release_until: int = 0
...
func _create_ui_elements() -> void:
	# Interact Prompt - Centered at bottom for 1080p
	...
	# Pickup Notification (independent popups) - Centered for 1080p
	...
	print("[UIManager-DEBUG] UI elements ready (centered for 1080p)")
...
# ─── Character Menu Auto-Save Hook ─────────────────────────────────────
func _on_character_menu_closed(menu_instance: CanvasLayer) -> void:
	print("[UIManager] Character menu closed → triggering auto-save")
	ui_closed.emit("character")
