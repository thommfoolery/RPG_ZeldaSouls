...
func use_current_quick_slot() -> void:
	...
	var item := EquipmentManager.get_equipped_item(slot_index)
	...
	if item.effect_type == "Damage" and item.trajectory_type != "Instant":
		...
		return   # Projectiles handle their own consumption
...
# ─────────────────────────────────────────────────────────────
# HANDLE_HEAL — special items only
# ─────────────────────────────────────────────────────────────
	...
	if item.special_component_ref != "":
		...
		if player and player.has_node(item.special_component_ref):
			...
			if component.has_method("_on_use_item_pressed"):
				...
				print("[QUICK-USE] Delegated to ", item.special_component_ref, " — it will handle everything")
				return
		push_warning("[QUICK-USE] Special component not found or missing method")
		return

	# Generic heal (Divine Blessing, potions, etc.)
...
func spawn_projectile(item: GameItem, slot_index: int) -> void:
	...
	# ─── REAL FACING DIRECTION FROM PLAYER (this was the missing piece) ───
	...
	elif player and "last_nonzero_dir" in player:   # AnimationComponent style
		dir = player.last_nonzero_dir
	elif player:
		dir = Vector2.RIGHT if player.scale.x > 0 else Vector2.LEFT

	var spawn_offset: Vector2 = Vector2(1, -1) * dir   # slightly in front + above hands
...
func _handle_teleport(_item: GameItem) -> void:
	print("[QUICK-USE] TELEPORT triggered")
